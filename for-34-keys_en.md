...
- Hold <kbd>SYM</kbd> and <kbd>ALT</kbd> together to activate the special symbols layer.
...
Symbols `<` `>` `«` `»` `[` `]` and ligatures `=>` and `->`, convenient for developers, are also available in the `ALT` layer for both languages.
